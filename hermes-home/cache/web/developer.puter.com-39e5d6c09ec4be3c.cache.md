On this page

# Free, Unlimited Nano Banana (Pro, 2, and 2 Lite) API

On this page
In this tutorial, we'll show you how to add Nano Banana image generation to your app for free, without any API keys. [Nano Banana](https://gemini.google/overview/image-generation/) is Google's lineup of image generation models that create high-quality images from text descriptions. Using [Puter.js](https://developer.puter.com), you can access its models, such as [Nano Banana 2 (Gemini 3.1 Flash)](/ai/google/gemini-3.1-flash-image/), [Nano Banana 2 Lite (Gemini 3.1 Flash Lite Image)](/ai/google/gemini-3.1-flash-lite-image/), [Nano Banana Pro (Gemini 3 Pro)](/ai/google/gemini-3-pro-image/), and [Nano Banana (Gemini 2.5 Flash Image)](/ai/google/gemini-2.5-flash-image/), directly from your frontend code, with no server-side setup or Google AI Studio setup.

Puter.js uses the [User-Pays model](https://docs.puter.com/user-pays-model/), where users of your application cover their own AI costs. Each user signs in with their own Puter account when they use the AI, so there are no API keys for you to manage. This means you as a developer don't pay anything for your users' usage, making your app practically free to run. You can scale to unlimited users and pay nothing for the AI or server usage.

## Getting Started

To use Puter.js, import our [npm module](https://www.npmjs.com/package/@heyputer/puter.js) in your project:

```js
 // npm install @heyputer/puter.js import { puter } from '@heyputer/puter.js' ;
```

Or alternatively, add our script via CDN if you are working directly with HTML, simply add it to the `<head>` or `<body>` section of your code:

```xml
 < script src = "https://js.puter.com/v2/" > </ script >
```

Nothing else is required to start using Puter.js for free access to Nano Banana image generation capabilities.

## Example 1: Image generation with Nano Banana Pro

Nano Banana Pro (Gemini 3 Pro Image) excels at generating sharp, legible text, allowing you to create posters or product mockups. It also comes with real-world knowledge and deep reasoning capabilities, ensuring your image information is accurate and precise.

```javascript
 puter. ai . txt2img ( "A vintage movie poster for 'The Last Voyage', featuring bold art deco typography with the tagline 'An Adventure Beyond Time' and 'Coming Soon 2024' at the bottom" , { model : "gemini-3-pro-image-preview" }) . then ( imageElement => { document . body . appendChild (imageElement); });
```

Full code example:

```html
 < html > < body > < h1 > Nano Banana Pro Image Generation </ h1 > < script src = "https://js.puter.com/v2/" > </ script > < script > puter. ai . txt2img ( "A vintage movie poster for 'The Last Voyage', featuring bold art deco typography with the tagline 'An Adventure Beyond Time' and 'Coming Soon 2024' at the bottom" , { model : "gemini-3-pro-image-preview" }) . then ( imageElement => { document . body . appendChild (imageElement); }); </ script > </ body > </ html >
```

## Example 2: Image generation with Nano Banana 2

Nano Banana 2 (Gemini 3.1 Flash Image) combines Pro-level quality with the speed of the Flash architecture, featuring advanced world knowledge, precise text rendering, and subject consistency.

```html
 < html > < body > < h1 > Nano Banana 2 Image Generation </ h1 > < script src = "https://js.puter.com/v2/" > </ script > < script > puter. ai . txt2img ( "A serene Japanese garden with cherry blossoms" , { model : "gemini-3.1-flash-image-preview" }) . then ( imageElement => { document . body . appendChild (imageElement); }); </ script > </ body > </ html >
```

## Example 3: Fast, low-cost image generation with Nano Banana 2 Lite

Nano Banana 2 Lite (Gemini 3.1 Flash Lite Image) is the fastest and lowest-cost model in the Nano Banana family, which makes it a good fit for high-volume or latency-sensitive apps and batch image generation.

```html
 < html > < body > < h1 > Nano Banana 2 Lite Image Generation </ h1 > < script src = "https://js.puter.com/v2/" > </ script > < script > puter. ai . txt2img ( "A red panda sleeping on a tree branch" , { model : "google/gemini-3.1-flash-lite-image" }) . then ( imageElement => { document . body . appendChild (imageElement); }); </ script > </ body > </ html >
```

## Example 4: Comparing Nano Banana models

You can compare different Nano Banana models to compare the results.

```html
 < html > < body > < h2 > Model Comparison </ h2 > < div id = "images" > </ div > < script src = "https://js.puter.com/v2/" > </ script > < script > const prompt = "A cyberpunk street scene with neon lights" ; const container = document . getElementById ( 'images' ); // Nano Banana Pro (Gemini 3 Pro Image) puter. ai . txt2img (prompt, { model : "gemini-3-pro-image-preview" }) . then ( img => { const div = document . createElement ( 'div' ); div. innerHTML = '<h3>Nano Banana Pro (Gemini 3 Pro Image)</h3>' ;
```

```html
 < html > < body > < h2 > Model Comparison </ h2 > < div id = "images" > </ div > < script src = "https://js.puter.com/v2/" > </ script > < script > const prompt = "A cyberpunk street scene with neon lights" ; const container = document . getElementById ( 'images' ); // Nano Banana Pro (Gemini 3 Pro Image) puter. ai . txt2img (prompt, { model : "gemini-3-pro-image-preview" }) . then ( img => { const div = document . createElement ( 'div' ); div. innerHTML = '<h3>Nano Banana Pro (Gemini 3 Pro Image)</h3>' ; div. appendChild (img); container. appendChild (div); }); // Nano Banana 2 (Gemini 3.1 Flash Image) puter. ai . txt2img (prompt, { model : "gemini-3.1-flash-image-preview" }) . then ( img => { const div = document . createElement ( 'div' ); div. innerHTML = '<h3>Nano Banana 2 (Gemini 3.1 Flash Image)</h3>' ; div. appendChild (img); container. appendChild (div); }); // Nano Banana 2 Lite (Gemini 3.1 Flash Lite Image) puter. ai . txt2img (prompt, { model : "google/gemini-3.1-flash-lite-image" }) . then ( img => { const div = document . createElement ( 'div' ); div. innerHTML = '<h3>Nano Banana 2 Lite (Gemini 3.1 Flash Lite Image)</h3>' ; div. appendChild (img); container. appendChild (div); }); // Nano Banana (Gemini 2.5 Flash Image) puter. ai . txt2img (prompt, { model : "google/gemini-2.5-flash-image" }) . then ( img => { const div = document . createElement ( 'div' ); div. innerHTML = '<h3>Nano Banana (Gemini 2.5 Flash Image)</h3>' ; div. appendChild (img); container. appendChild (div); }); </ script > </ body > </ html >
```

Collapse code

## Example 5: Text and Image-to-Image generation

The following example shows how to generate an image from a text prompt and an image by providing `input_image` and `input_image_mime_type` parameters:

```html
 < html > < body > < script src = "https://js.puter.com/v2/" > </ script > < script > puter. ai . txt2img ( "Turn this image into a watercolor painting" , { model : "google/gemini-3.1-flash-image-preview" , input_image : "" , input_image_mime_type : "image/png" }). then ( ( image )=> { document . body . appendChild (image); }); </ script > </ body > </ html >
```

## Available Models

Puter.js gives you access to the following Google image models through the [`puter.ai.txt2img()`](https://docs.puter.com/AI/txt2img/) function:

```undefined
 google/gemini-3.1-flash-lite-image google/gemini-3.1-flash-image-preview google/gemini-3-pro-image-preview google/gemini-2.5-flash-image google/imagen-4.0-ultra google/imagen-4.0 google/imagen-4.0-fast google/imagen-4.0-preview
```

## Nano Banana Free API Key

Google via their AI Studio gives you an API key for free, with no credit card required. That key works with a limited selection of their models, so some use cases are free to build. Nano Banana is not one of them: it sits outside the free tier, so once you generate an image you have to enable billing and pay per image. Creating the key costs nothing; using it for image generation does.

With Puter.js, you can add Nano Banana to your app without an API key. There is nothing to create and no billing to enable, and each user covers their own image generation through their Puter account, so your app runs at no cost to you regardless of how many users you have.

## Conclusion

To add Nano Banana to your app for free, include Puter.js and call `puter.ai.txt2img()` with a prompt and a Gemini image model. There are no Google API keys, no billing to enable, and no backend to set up. Through the [User-Pays model](https://docs.puter.com/user-pays-model/), each user covers their own image generation, so your app stays free to run as it scales to any number of users.

You can find all AI features supported by Puter.js in the [documentation](https://docs.puter.com/AI/).

- [Free, Unlimited Image Generation API](/tutorials/free-unlimited-image-generation-api/)
- [Free, Unlimited Imagen API](/tutorials/free-unlimited-imagen-api/)
- [Free, Unlimited GPT Image API](/tutorials/free-unlimited-gpt-image-api/)
- [Free, Unlimited Stable Diffusion API](/tutorials/free-unlimited-stable-diffusion-api/)
- [Free, Unlimited Google AI API](/tutorials/free-unlimited-google-ai-api/)
- [Free, Unlimited Gemma API](/tutorials/free-unlimited-gemma-api/)
- [Gemini API Pricing](/tutorials/gemini-api-pricing/)
- [Free, Unlimited ByteDance Seedream API](/tutorials/free-unlimited-bytedance-seedream-api/)
- [Free, Unlimited Leonardo.Ai API](/tutorials/free-unlimited-leonardo-ai-api/)
- [Free, Unlimited Ideogram API](/tutorials/free-unlimited-ideogram-api/)
- [Ideogram API Pricing](/tutorials/ideogram-api-pricing/)
- [Free, Unlimited HiDream API](/tutorials/free-unlimited-hidream-api/)
- [Free, Unlimited RunDiffusion API](/tutorials/free-unlimited-rundiffusion-api/)
- [Free, Unlimited OpenAI API](/tutorials/free-unlimited-openai-api/)
- [Free, Unlimited Claude API](/tutorials/free-unlimited-claude-35-sonnet-api/)
- [Free, Unlimited AI API](/tutorials/free-unlimited-ai-api/)
- [Free, Unlimited OpenRouter API](/tutorials/free-unlimited-openrouter-api/)
- [Free, Unlimited OCR API](/tutorials/free-unlimited-ocr-api/)
- [Free, Unlimited Text to Speech API](/tutorials/free-unlimited-text-to-speech-api/)

## Ship a Full-Stack App with One Prompt

Give this to your AI `Build an AI chat app using Puter.js`

Try in

Coding manually? [see the guide](https://docs.puter.com/getting-started/)